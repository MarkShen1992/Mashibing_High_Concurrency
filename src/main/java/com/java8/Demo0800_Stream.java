...
public class Demo0800_Stream {
    public static void main(String[] args) {
        List<Person> persons = Arrays.asList(
            new Person(1, "Bob", 21, 1),
            new Person(2, "Alice", 25, 0),
            new Person(4, "Calvin", 26, 1),
            new Person(3, "Susan", 34, 0),
            new Person(5, "Trump", 70, 1)
        );
        ...
        List<Person> pagedPerson = persons.stream()
            .skip(recordIndex) // 每页第一条数据的位置
            .limit(pageSize) // 页面大小
            .collect(Collectors.toList());
        ...
        List<Person> sortedPersons = persons.stream()
            .sorted(Comparator.comparing(Person::getAge).reversed()) // 升序
            .collect(Collectors.toList());
        ...
        sum = persons.stream()
            ...
            .reduce(0, (total, currentVal) -> total + currentVal);
        ...
        // calculating summary
        IntSummaryStatistics ageStatistics = persons.stream()
            .mapToInt(Person::getAge)
            .summaryStatistics();
        ...
        // Grouping
        Map<Integer, List<Person>> peopleGroupBySex = persons.stream()
        ...
        Map<Integer, Double> averageAgeByGender = persons.stream()
